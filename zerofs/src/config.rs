...
impl Settings {
    ...
    pub fn write_default_config(path: &str) -> Result<()> {
        ...
        // Add concurrent writers section
        toml_string.push_str("\n# ============================================================================\n");
        toml_string.push_str("# MULTIPLE WRITERS (OPTIONAL)\n");
        toml_string.push_str("# ============================================================================\n");
        toml_string.push_str("# Set to 'true' if you need to run multiple ZeroFS instances that write\n");
        toml_string.push_str("# to the same storage bucket at the same time (e.g., multiple servers,\n");
        toml_string.push_str("# CI/CD workers, or distributed processing).\n");
        toml_string.push_str("#\n");
        toml_string.push_str("# Requirements:\n");
        toml_string.push_str("#  - Only works with S3, Google Cloud Storage, or Azure Blob Storage\n");
        toml_string.push_str("#  - Leave as 'false' (default) if only one ZeroFS writes at a time\n");
        toml_string.push_str("#\n");
        toml_string.push_str("# [storage]\n");
        toml_string.push_str("# allow_concurrent_writers = true\n");
