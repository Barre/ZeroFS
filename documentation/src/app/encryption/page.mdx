...
ZeroFS validates the password before touching the store: at `zerofs run` startup (the server refuses to start), in `zerofs change-password`, and in the standalone compaction worker and debug commands.
