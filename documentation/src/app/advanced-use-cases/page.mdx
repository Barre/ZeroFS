...
- **Flush latency**: With `wal_enabled = true`, a flush is a WAL append, so flush latency follows the WAL store's latency. A [separate WAL store](/separate-wal) on lower-latency storage reduces it.
