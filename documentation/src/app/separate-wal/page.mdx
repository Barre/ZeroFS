...
With the WAL enabled, every `fsync` writes to the WAL, so fsync latency equals the latency of the WAL's backing store. You can point the WAL at a separate, lower-latency store. {{ className: 'lead' }}
