...
- **No File Locking**: ZeroFS doesn't support NFS locking protocols. `fcntl` byte-range locking is available over 9P, from both the kernel client and `zerofs mount`; applications that need locks (SQLite, other embedded databases) should mount that way. See [File Locking](/9p-access#file-locking).
