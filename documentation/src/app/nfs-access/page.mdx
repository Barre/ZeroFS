...
ZeroFS serves its filesystem over NFSv3. macOS, Linux, and Windows mount it with the NFS clients that ship with the operating system. {{ className: 'lead' }}
...
- **No File Locking**: ZeroFS doesn't support NFS locking protocols. `fcntl` byte-range locking is available through the [native kernel client](/kernel-client), stock v9fs, and `zerofs mount`; applications that need locks (SQLite, other embedded databases) should mount over 9P. See [File Locking](/9p-access#file-locking).
- **UID/GID Mapping**: NFS uses numeric IDs, not usernames
- **Stateless Protocol**: The server keeps no per-client state
