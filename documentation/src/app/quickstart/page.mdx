...
# Extract the archive
tar -xzf zerofs-pgo-multiplatform.tar.gz zerofs-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m)-pgo

# Make the binary executable and move it to PATH
chmod +x zerofs-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m)-pgo
sudo mv zerofs-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m)-pgo /usr/local/bin/zerofs

# Clean up the archive
rm zerofs-pgo-multiplatform.tar.gz
rm zerofs-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m)-pgo
```
