...
The 9P server already handles permissions, streaming, renames, and directory listings, so the UI reuses it directly through a [9P client in TypeScript](https://github.com/Barre/zerofs/tree/main/webui/src/lib/ninep) over a WebSocket rather than a REST layer that would reimplement the same logic.
