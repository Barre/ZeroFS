export const metadata = {
  ...
  description:
    'The private extensions ZeroFS negotiates on top of 9P2000.L: fewer round trips, idempotent retries, reconnect by inode id, and durability-verified fsync, with their on-the-wire framing.',
}
...
ZeroFS serves standard [9P2000.L](/9p-access), the Linux dialect of the protocol. Its own client negotiates a set of private extensions on top of it: fewer round trips on metadata-heavy work, idempotent retry across a reconnect, fid rebinding that survives a server restart, and an fsync that verifies durability rather than assuming it. This page documents what each one does and how it is framed on the wire. {{ className: 'lead' }}
...
This is the protocol-level half of the [durability and consistency](/durability) guarantee, and it is what lets the bundled client verify fsync durability through a [high-availability](/high-availability) failover. A server with `ignore_fsync` set never offers level 4, so a client that asks for verified durability is never silently downgraded to an unverified success.
