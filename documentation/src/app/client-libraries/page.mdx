...
An open handle with no remaining namespace link is connection-local. Disconnecting with such a handle makes the logical client terminal with `Stale`/`ESTALE`; recovery requires a new client and reopened paths. In-flight mutations retain their operation envelope across retries. Automatic retry ends at the 120-second protocol horizon, after which an unresolved result is reported as an ambiguous connection failure.
