...
The wheel bundles the native library (manylinux and musllinux on Linux, plus macOS and Windows), so the install is self-contained (no Rust toolchain, no build step). Import `zerofs_client` for the idiomatic facade; the low-level uniffi module is also importable as `zerofs_ffi`.
