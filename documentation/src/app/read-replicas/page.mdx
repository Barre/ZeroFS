...
- **Metadata metrics**: The `/metrics` endpoint on a replica serves ZeroFS metrics but omits the `slatedb_`-prefixed series, which come from the read-write engine. See [Prometheus Metrics](/prometheus).
