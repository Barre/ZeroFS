...
The writer runs a dedicated segment GC task: one pass at startup, then one per interval. The interval adapts: 60 seconds (configurable) while the store is active or the backlog fits in one pass, dropping to 5 seconds only while two conditions hold at once: the previous pass hit a work budget with actionable backlog remaining, and the store was completely idle since that pass (zero bytes read or written, zero mutations, no seal in flight). While those conditions continue to hold mid-pass, the pass also repeats its selection and compaction step back-to-back up to 32 batches of the per-batch budgets below, re-checking for client activity between batches and yielding within one batch of any arriving operation so the barrier and counter scan amortize across an idle drain instead of recurring per 256 MiB. Any client activity, any error, or a drained backlog returns the cadence to the base interval immediately, and the loop never sleeps longer than the base interval. A pass begins by sealing the open segment buffer and flushing the metadata store under the flush barrier, so the durable view matches the in-memory view; a segment GC pass is therefore also a flush trigger. On a fast idle pass the barrier is a near no-op because the idle test admitted no writes since the last one; the pass's cost is the reclamation work itself — which can be a full round of gathers, segment PUTs, and deletes — plus about two small bookkeeping PUTs of fixed overhead (the previous pass's own commits flushing).
...
| Parameter | Value |
|-----------|-------|
| Pass interval | adaptive: 60 s base / 5 s while saturated and idle (both configurable) |
...
| Compaction budget | 64 segments / 256 MiB per batch; up to 32 batches per pass while the store stays idle |
...
Values are configurable through the `[gc]` section (see [Configuration](/configuration)): the base pass interval, the idle drain interval, the tail-scrub floor, and the [read-directed](/read-directed-compaction) switch (`read_directed`, default true).
