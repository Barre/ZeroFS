...
Segment GC exports no Prometheus counters. Each pass logs one info-level summary line: segment count, total and live bytes, reclaimable bytes, segments deleted, frames relocated, segments tail-scrubbed, and the [read-directed](/read-directed-compaction) fields (nominations drained and selected, hot seams, and chains packed out of chains assembled, with unpacked chains counted by cause: deferred, warm, or over-reserve). The loop additionally logs each cadence shift between the base and idle intervals. Its effect is otherwise visible as the bucket's object count and size under `segments/`.
