...
Start ZeroFS with NBD support using either TCP or Unix socket (or both):
...
- `ZEROFS_NBD_HOST` - Bind address for NBD TCP server (default: 127.0.0.1)
- `ZEROFS_NBD_PORT` - Port for NBD server (default: 10809)
- `ZEROFS_NBD_SOCKET` - Unix socket path for NBD (optional)
