export const metadata = {
  ...
  description: 'Expose ZeroFS storage as raw block devices over NBD for filesystems, ZFS pools, databases, and VM disks.',
}

# NBD Block Devices

ZeroFS runs a Network Block Device (NBD) server that exposes S3 storage as raw block devices. The devices hold ext4 filesystems, ZFS pools, databases, or VM boot disks. TRIM/discard is supported. {{ className: 'lead' }}
...
- **TRIM Support** - Discard deletes the extent pointers from the metadata store; segment garbage collection then reclaims the space in S3
...
- `addresses` - Array of TCP bind addresses. If omitted, no TCP listener is started (use `unix_socket` instead). The `zerofs init` template sets `["127.0.0.1:10809"]`
...
- **FLUSH** - Flushes the entire filesystem: seals the open segment buffer and uploads it to the object store, then flushes the metadata memtable. The reply is sent only after both complete. Concurrent FLUSH and FUA requests coalesce into a single flush. Flush latency includes the segment upload: up to 256 MiB of writes buffered since the last seal.
...
See [Durability](/durability) for what durable means under each configuration.
...
1. ZeroFS deletes the extent pointers from the metadata store and debits each affected segment's live-byte counter
2. Segment garbage collection: every 60 seconds, or every 5 seconds while a saturated backlog meets an idle store. The task deletes segments with no live bytes and repacks fragmented ones (live data below 50%, or segments under 1 MiB). Write-cold segments above the scrub floor of dead bytes are repacked with leftover budget.

See [Garbage Collection](/garbage-collection) for the full reclamation pipeline.

### WRITE_ZEROES vs TRIM

WRITE_ZEROES routes zero data through the normal write path in 1 MiB pieces. The write path elides any resulting all-zero extent into a pointer delete, so extent-aligned zeroing (for example `blkdiscard --zeroout` over a whole device) frees object-store space the same way TRIM does. TRIM is still the cheaper command: it deletes extent pointers directly, without pushing zero buffers through the write pipeline, and it skips extents beyond end of file. Both free space at 32 KiB extent granularity; a partially covered extent is zeroed in place and freed only if the whole extent becomes zero. Prefer plain `blkdiscard`, `fstrim`, or `zpool trim` / `autotrim`.
...
A connected client keeps the size negotiated at connect time; the server re-reads the device file's size at every connection. To resize, disconnect the client, truncate the device file to the new size, and reconnect. Growing preserves existing data; shrinking discards data beyond the new size.

```bash
# Disconnect the NBD client
nbd-client -d /dev/nbd0
# Resize the device file (growing preserves data)
truncate -s 2G /mnt/zerofs/.nbd/database
# Reconnect; the new size is negotiated at connect time
nbd-client 127.0.0.1 10809 /dev/nbd0 -N database -persist -timeout 600 -connections 4
```
...
<div className="not-prose mt-6 flex gap-3">
  <Button href="/advanced-use-cases" arrow="right">
    Advanced use cases
  </Button>
