...
Set both `replication_listen` and `peers` on every node so either can lead after a role swap. ZeroFS rejects a configuration that sets only one of them; a standalone node may omit both. Acknowledged writes that have not been `fsync`'d survive failover: the leader ships them to the standby, which rebuilds any missing segment objects at takeover. For a workload that tolerates the risk, `[filesystem] ignore_fsync = true` makes `fsync` a no-op and relies on the standby for durability.
