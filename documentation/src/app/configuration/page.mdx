...
```bash
# NBD TCP server configuration
export ZEROFS_NBD_PORT='10809'      # Port (default: 10809)
export ZEROFS_NBD_HOST='0.0.0.0'    # Bind address (default: 127.0.0.1)

# NBD Unix socket (optional, can be used alongside TCP)
export ZEROFS_NBD_SOCKET='/tmp/zerofs-nbd.sock'

# Note: NBD devices are managed dynamically via the .nbd/ directory
# Create devices: truncate -s <size> /mnt/zerofs/.nbd/<device-name>
```
...
export ZEROFS_9P_SOCKET='/tmp/zerofs.sock'  # Unix socket path (optional, enables Unix socket server)
```
...
export ZEROFS_NBD_PORT='10809'
# Optional: Use Unix socket for better local performance
# export ZEROFS_NBD_SOCKET='/tmp/zerofs-nbd.sock'
