...
<Note>
`memory:///` keeps all data in process memory: the store starts empty, everything is lost when the process exits, and it cannot be shared with a standalone compaction worker. Use it for throwaway testing only.
</Note>
...
The admin endpoint for `zerofs checkpoint`, `zerofs flush`, `zerofs monitor`, `zerofs fatrace`, and `zerofs otrace`. These commands read the same configuration file, try the Unix socket first if the socket file exists, then each address in order. Without a `[servers.rpc]` section, they exit with `RPC server not configured in config file`. See [Monitoring](/monitoring) and [Checkpoints](/checkpoints).
