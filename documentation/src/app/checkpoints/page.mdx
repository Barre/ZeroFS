export const metadata = {
  title: 'Checkpoints',
  description:
    'Named checkpoints capture a ZeroFS filesystem at a point in time. Any checkpoint can be opened read-only.',
}

# Checkpoints

Named checkpoints capture the filesystem at a point in time. Any checkpoint can be opened read-only with a single flag at startup. {{ className: 'lead' }}

## Overview

Checkpoints are named snapshots of the filesystem state. They're useful for:
...
<Row>
  <Col>
    ...
    Checkpoints are created via RPC commands sent to the running ZeroFS process, which coordinates the snapshot so it captures a consistent state.
