...
Connection-loss behavior differs. The kernel client does not rebuild a session after a server restart or network interruption; operations fail until the filesystem is unmounted and remounted. `zerofs mount` retries indefinitely with 50–500 ms backoff and rebuilds the session. Undispatched operations block until a server is available.

Session replay rebinds linked open handles by stable inode id and re-acquires recorded byte-range locks. An open file with no remaining namespace link is connection-local and cannot be replayed. Its presence during reconnect makes the logical mount session terminal with `ESTALE`; recovery requires a new mount.
...
- Because the client and server ship together, they negotiate [private 9P extensions](/9p-extensions) the kernel client doesn't have, including metadata fast paths and atomic fallocate.
...
Locks live in server memory, not in the object store. A client's locks are released when its connection closes, and a server restart or HA takeover starts with an empty lock table. `zerofs mount` records its locks and re-acquires the complete set before publishing a rebuilt session; the kernel client does not, since its mount is wedged after a disconnect. Another session may acquire a lock during the gap, so replay is not continuous ownership and advisory locks are not a distributed fencing primitive.
