...
ZeroFS implements a transactional storage model using an LSM-tree database designed for object storage backends. This architecture provides three properties:
...
<Row>
  ...
  <Col>
    ...
    No scanning or repair is performed. With the WAL enabled, only entries since the last SST flush are replayed. The time to resume operation is independent of filesystem size.
...
<Row>
  <Col>
    ...
    - `fsync()` issues a durability-verified flush (`Tfsyncdur`)
    - ZeroFS flushes all buffered data to object storage, then verifies the writes' lineage
    - The call returns success only when durability is confirmed, otherwise `ESTALE` (see [Verified fsync](#verified-fsync))
...
In addition to crash simulation, CI runs pjdfstest, xfstests, stress-ng, Linux kernel builds, Jepsen's local-fs suite, and a ZFS integration test on every commit.
...
<Row>
  <Col>
    ...
    **xfstests**: The Linux kernel's filesystem test suite, originally developed for XFS but now used across all major Linux filesystems. Covers edge cases, concurrent operations, and error handling.
...
| xfstests | NFS, 9P, FUSE | Linux filesystem semantics |
