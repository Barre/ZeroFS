...
ZeroFS stores file contents in immutable segment objects written directly to the object store. An LSM tree on the same object store holds only metadata: inodes, directory entries, and one 32-byte pointer per extent. This page documents the on-object format, the write and read paths, and space reclamation. {{ className: 'lead' }}
...
A garbage-collection pass runs on the writer: every 60 seconds while the store is active, every 5 seconds while a budget-saturated backlog meets a completely idle store (both intervals configurable, see `[gc]` under [Configuration](/configuration)):
...
4. **Pack fragmented segments**: segments under 50% live or smaller than 1 MiB are repacked into fresh segments targeting 256 MiB, at most 64 sources and 256 MiB per round. Live frames are fetched via coalesced ranged GETs, re-sorted by (inode, extent index) for physical contiguity, sealed durably, then repointed with a conditional swap under the per-inode lock. Drained sources are not deleted in the same pass; they become dead and are removed later, so in-flight reads stay valid. Leftover round budget is spent scrubbing the tail: write-cold segments above the scrub floor (default 5%) of dead bytes that neither predicate would ever select; without this, dead space in never-touched-again segments would be stranded permanently. A dense segment with no dead bytes is rewritten by exactly one path: a hot-seam chain repack ([Read-Directed Compaction](/read-directed-compaction)).

Directory reads during GC are two ranged GETs (the 64-byte footer suffix, then the directory span), never the whole object.

File data no longer rides LSM compaction. The embedded compactor still runs in `zerofs run`, but rewrites only metadata SSTs. Deleted-file extent pointers are removed by the tombstone sweep described on the [Garbage Collection](/garbage-collection) page; the segment bytes are then reclaimed by the pass above.

## Design Constants {{ id: 'design-constants' }}

Apart from the four `[gc]` keys ([Configuration](/configuration)), these values are compile-time constants:
