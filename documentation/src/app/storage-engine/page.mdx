...
export const metadata = {
  ...
  description:
    'How ZeroFS stores file data: 32 KiB extents sealed as compressed, encrypted frames inside immutable segment objects, with 32-byte FrameLoc pointers in SlateDB.',
}
...
ZeroFS stores file contents in immutable segment objects written directly to the object store. SlateDB holds only metadata: inodes, directory entries, and one 32-byte pointer per extent. This page documents the on-object format, the write and read paths, and space reclamation. {{ className: 'lead' }}
...
| SlateDB (LSM on the same object store) | Inodes, directory entries, tombstones, stats; one 32-byte FrameLoc per extent; per-segment live-byte counters |
...
The **FrameLoc** is the SlateDB value of an extent key: 32 bytes, little-endian.
...
`fsync` (and every other flush) runs a fixed sequence: drain in-flight seals, re-upload any failed seal, seal the current open buffer, then flush SlateDB. If sealing fails, the metadata flush does not run. The invariant: **a durable metadata manifest never references an un-uploaded segment**. A crash between segment upload and metadata flush leaves an orphaned segment object, which garbage collection reclaims — never a durable pointer to a missing object. This invariant is exercised by failpoint crash tests.

SlateDB's write-ahead log is permanently off and not configurable: a WAL flush is not gated by the seal barrier and could persist a FrameLoc to a segment that was never uploaded.
...
SlateDB tracks live bytes per segment. Transactions carry signed deltas; the single-writer commit worker folds them into absolute counters. Overwriting or deleting an extent debits the segment that held its old frame.
...
File data no longer rides LSM compaction. SlateDB compaction still runs, embedded in `zerofs run`, but rewrites only metadata SSTs. Deleted-file extent pointers are removed by the tombstone sweep described on the [Garbage Collection](/garbage-collection) page; the segment bytes are then reclaimed by the pass above.
...
`[lsm] wal_enabled` and `[lsm] max_unflushed_gb` no longer exist; a configuration that sets either fails to parse. Cache sizing (`[cache] disk_size_gb`, `memory_size_gb`, `warm_metadata`) is covered on the [Caching](/caching) page.
