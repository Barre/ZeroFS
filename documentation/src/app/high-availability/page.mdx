...
**One writer at a time.** ZeroFS is single-writer per database. Exactly one node, the leader, holds the data-database writer; the standby does not write to it until it takes over. At startup a node asks its peer who is active rather than trusting its configured role, so a restarting node defers to a live peer instead of fencing it. Each writer also names itself in a marker object beside the database (`.zerofs_ha_leader`) before serving; a booting node that finds its own name there does not open as writer while its peer is silent, since a silent peer may be a live partition holding acked writes or an already-promoted leader, and a configured standby likewise never opens from silence (see [Configuration](#configuration) for the operator override). Where two nodes do race to open, the epoch fence (below) guarantees at most one commits.
...
**Semi-synchronous replication, commit then apply.** For each write the leader ships it to the standby and waits for the standby's acknowledgment before applying it locally and acknowledging the client. Anything a client can read has therefore already been replicated, so the standby is never missing a write a client has seen. The standby keeps not-yet-applied writes in an in-memory tail buffer, pruned as the leader's flushes confirm batches durable, and replays them in order when it takes over. A takeover validates the replay: every replicated batch commits a stamp recording how far shipping had gotten and whether the leader kept committing alone after its last acked ship, so the replay applies only batches that extend the database's durable state, and discards a tail the dead leader had outlived (its content re-committed or superseded while Solo) or one left over from an older leader term, instead of overwriting newer data with it.
...
| --- | --- | --- |
| `node_id` | yes | Stable identity of this node, unique within the pair: it keys the latest-writer record, so startup fails when a peer reports the same `node_id`. |
...
**Silence never elects a writer.** Two kinds of node wait at startup rather than open against a silent peer, logging a warning while they wait: the node that most recently opened the database as writer (named in the `.zerofs_ha_leader` marker object beside it), whatever its configured role, and any configured standby, record or no record. Any answer resolves the role: a peer that is serving or holds an un-replayed tail takes over and the booting node follows; an idle peer means there is nothing to preserve and the configured roles apply. Only silence blocks, because silence cannot distinguish a dead peer, which costs nothing, from a partitioned live one, which holds acked writes or is already leading. If the peer is permanently gone, remove it from `peers`, set `role = "leader"` if this node had `"standby"`, and restart; that edit asserts the peer is dead, so verify the host is actually down first, since booting solo against a live partitioned peer fences it. The one node that boots through silence is a configured leader the record does not name: that is first bring-up of a fresh database, which therefore needs no peer reachable, and the recovery path after both nodes were lost.
...
- **Restarting a node alone may need an operator.** The recorded latest writer, and any configured standby, wait rather than serve while their peer is unreachable, until it answers or the config is edited (see [Configuration](#configuration)). This is deliberate: no timeout can distinguish a dead peer from a partition, and guessing wrong either abandons acked writes on the peer or fences a live leader. The exception is a configured leader the record does not name, which boots through silence: first bring-up, and the recovery after losing both nodes.
