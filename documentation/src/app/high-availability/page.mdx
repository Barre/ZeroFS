...
**There is no WAL.** The write-ahead log is permanently off and not configurable; the former `[lsm] wal_enabled` key no longer exists, and a config that sets it fails to parse. A WAL flush is not gated by the segment seal barrier, so it could make an extent pointer durable while the segment it points into is still an un-uploaded buffer. Protection for un-`fsync`'d acked writes comes from semi-synchronous frame shipping instead (see [Durability and Consistency](/durability)).
